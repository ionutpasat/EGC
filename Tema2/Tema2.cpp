...
float Tema2::length(glm::vec3 p1, glm::vec3 p2) {
...
void Tema2::RenderScene(float deltaTimeSeconds) {
	{
		...
		if (length(p2, p1) <= length(firstCarPos, p1)) {
			firstCarPos = p2;
			car1Pos++;
		}
		if (length(p22, p21) <= length(secCarPos, p21)) {
			secCarPos = p22;
			car2Pos++;
		}
		if (length(p32, p31) <= length(thCarPos, p31)) {
			thCarPos = p32;
			car3Pos++;
		}
...
void Tema2::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_W)) {
		...
		if (length(pos, firstCarPos) <= 0.6f || length(pos, secCarPos) <= 0.6f || length(pos, thCarPos) <= 0.6f) {
			return;
		}
		else {
