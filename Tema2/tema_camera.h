...
namespace implemented
{
    class HwCamera
    {
     public:
        ...
        HwCamera(const glm::vec3 &position, const glm::vec3 &center, const glm::vec3 &up)
        {
        ...
        {
            // Translates the HwCamera using the `dir` vector computed from
            // `forward`. Movement will always keep the HwCamera at the same
        ...
        {
            // TODO(student): Translate the HwCamera using the `forward` vector.
            // What's the difference between `TranslateForward()` and
            // `MoveForward()`?
            position += glm::vec3(forward.x, 0 , forward.z) * distance;
        }

        void TranslateUpward(float distance)
        {
            // TODO(student): Translate the HwCamera using the `up` vector.
        ...
        void TranslateRight(float distance)
        {
            ...
            // You need to translate the HwCamera using the `right` vector.
            // Usually, however, translation using HwCamera's `right` vector
            // is not very useful, because if the HwCamera is rotated around the
            // `forward` vector, then the translation on the `right` direction
            // will have an undesired effect, more precisely, the HwCamera will
        ...
        void RotateFirstPerson_OX(float angle)
        {
            ...
            glm::mat4 rotation = glm::rotate(glm::mat4(1.f), angle, right);
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            glm::mat4 rotation = glm::rotate(glm::mat4(1.f), angle, forward);
        ...
        {
            // TODO(student): Rotate the HwCamera in third-person mode around
